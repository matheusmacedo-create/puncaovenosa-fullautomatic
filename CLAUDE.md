...
- O passo 1 da triagem guarda `{ cep, bairro, cidade, uf, latitude, longitude }`, não só o CEP. Os leitores (`lib/cep.ts`) aceitam a forma antiga em texto puro — há rascunho em `localStorage` de quem começou antes. A consulta é pela nossa rota `/api/cep/[cep]`, nunca do navegador direto para a ViaCEP ou o Nominatim. `latitude`/`longitude` idealmente vêm da BrasilAPI (coordenada exata do CEP), mas nem sempre ela devolve — e a ViaCEP nunca devolve. Quando falta, `PUT /api/triagem` completa com uma coordenada **aproximada** por bairro/cidade via Nominatim (`coordenadaAproximada` em `lib/cep.ts`) antes de gravar — de propósito ali, e não na busca em si, porque o aluno não espera essa gravação (`save()` no front é fire-and-forget). É o que alimenta o mapa de origem em `/secretaria`; só fica sem ponto quem não tem nem cidade identificável.
