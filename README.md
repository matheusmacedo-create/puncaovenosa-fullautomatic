...
O funil e a redação usam o **mesmo** projeto Supabase: `puncaovenosa-fullautomatic` (ref `lqpnbqislaxzhqkszijg`, região `sa-east-1`, São Paulo), schema `public`. Foi uma consolidação deliberada: dois projetos Supabase para uma instituição só era um a mais para pagar, monitorar e esquecer. A direção foi decidida pela região: o projeto da redação (`RedacaoCruzVermelha-Rj`, ref `wlbbfkudeibalkaqphpo`) tinha sido criado no Canadá (`ca-central-1`), região de projeto Supabase não se troca depois de criada, e cada ida ao banco a partir da função em `gru1` pagava ~130 ms a mais lá do que paga aqui. Então foi a redação que veio: as 23 tabelas dela, o schema `private` com as 5 funções auxiliares da RLS, as 74 policies, os 3 RPCs, gatilhos e índices foram aplicados aqui (migrations `cvrj_01`, `cvrj_02` e `cvrj_03` no histórico do projeto, reproduzindo os 21 arquivos de `supabase/migrations` do repositório `redacao-cruzvermelhariodejaneiro`), e os dados — inclusive os usuários em `auth.users`/`auth.identities` — foram copiados banco a banco, sem passar por arquivo, e conferidos por checksum linha a linha nas 20 tabelas com conteúdo. A migration `0016` faz a blindagem que um projeto compartilhado exige: tira `anon`/`authenticated` de tudo que é do funil (a chave publicável da redação está no bundle do navegador dela) e concede ao `service_role`, explicitamente, o que o servidor precisa.

Regras que valem daqui em diante, para os dois repositórios: nome novo de tabela, função, tipo ou sequence é conferido contra o que o outro sistema já tem antes de virar migration; privilégio é concedido tabela a tabela, nunca `on all tables in schema public` (a migration 2 da redação fazia isso e foi aplicada aqui com a lista explícita); e cada repositório só versiona os próprios objetos — os da redação vivem em `redacao-cruzvermelhariodejaneiro/supabase/migrations`, os do funil aqui.

O projeto do Canadá fica como arquivo até a redação ser confirmada apontando para cá; depois pode ser apagado. Os objetos do funil que tinham sido copiados para lá já foram removidos, e a redação continua intacta lá até a troca.

**Troca das variáveis (a única etapa manual), no projeto da redação na Vercel — o funil não muda nada:** `NEXT_PUBLIC_SUPABASE_URL` e `SUPABASE_URL` passam a `https://lqpnbqislaxzhqkszijg.supabase.co`; `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` passa a `sb_publishable_T-hfk73P8u6mqNL-zoNTJQ_v5neKHP_` (se o projeto ainda usa `NEXT_PUBLIC_SUPABASE_ANON_KEY`, é a chave `anon` deste projeto); `SUPABASE_SERVICE_ROLE_KEY` passa a ser a chave secreta deste projeto (Dashboard → Project Settings → API Keys). Depois, **Redeploy**. Quem estava logado na redação precisa entrar de novo — o cookie de sessão leva o ref do projeto no nome, então a sessão antiga simplesmente deixa de existir; as senhas continuam as mesmas. Linhas criadas no Canadá entre a cópia e a troca são trazidas por uma nova rodada da mesma cópia (é idempotente) — peça isso logo depois de trocar.
...
O pixel do navegador tem um limite físico: bloqueador de anúncio, Safari/ITP e navegador com rastreamento restrito derrubam uma fatia real dos eventos sem ninguém perceber que sumiram — o Meta nunca avisa "faltaram eventos", só reporta um número mais baixo do que o real. `lib/meta-capi.ts` manda uma segunda cópia, pelo servidor, dos quatro eventos que nascem de uma transição que o back-end já enxerga: `dados` (Lead), `pagamento` (AddPaymentInfo), `pago` (Purchase) e `triagemFim` (CompleteRegistration). `funil_2_cta` e os dois marcos sem evento padrão (`triagemInicio`, `ficha`) continuam só no pixel — são clique e navegação, não têm uma escrita no banco para ancorar o envio do servidor.

**Cada chamada manda os dois nomes, não só o padrão.** O navegador dispara `trackCustom(nome_da_etapa, ...)` e `track(evento_padrão, ...)` juntos, do mesmo clique — dois eventos distintos aos olhos do Meta, cada um com sua própria pontuação de qualidade de correspondência no Gerenciador. Mandar a cópia do servidor só sob o nome padrão (`Lead`, `Purchase`, ...) deixava o nome próprio (`funil_3_dados`, `funil_5_pago`, ...) — que é o que desenha o funil completo, e o que os relatórios internos usam — sem nenhum reforço do servidor, preso na pontuação mais baixa que só o pixel sozinho consegue. `enviar()` monta os dois objetos no mesmo `POST`, cada um com seu próprio `event_id` (mas o mesmo valor de id em ambos, porque é assim que o navegador também manda), então cada par (nome, id) dedupinca certo com o par equivalente do pixel, sem os dois nomes se confundirem entre si.
...
**O que o servidor manda que o navegador não tem, ou nem sempre captura a tempo:** e-mail, telefone e CPF da inscrição, com hash SHA-256 (`em`, `ph`, `external_id` — a correspondência avançada do Meta), além de nome (`fn`/`ln`) e país fixo (`country: 'br'` — o curso é presencial na sede do Rio, então é seguro afirmar mesmo sem endereço ainda). Quando a inscrição já passou pelo passo 1 da triagem (só o `CompleteRegistration` chega tão longe), soma-se cidade, UF e CEP (`ct`/`st`/`zp`) — cidade sem acento antes do hash, porque "São Paulo" e "Sao Paulo" têm hash diferente e o Meta não normaliza isso por conta própria. O CPF em especial nunca passa pelo pixel — o navegador não tem essa informação nesse formato em lugar nenhum. Quando a requisição é do próprio navegador do aluno (dados, pagamento, confirmação manual, consulta de status, triagem), soma-se IP, user-agent e os cookies `_fbp`/`_fbc` — este último só existe quando a visita veio de um clique em anúncio (tem `fbclid` na URL); numa inscrição de teste feita direto, sem vir de anúncio, o Gerenciador de Eventos aponta "fbc ausente" como se fosse um problema nosso, mas é esperado: não tem `fbc` pra mandar porque não houve clique de anúncio algum. **Exceção proposital:** o webhook da Únicopag (`app/api/webhooks/unicopag/route.ts`) não passa IP/user-agent/`_fbp`/`_fbc` — a requisição ali vem do servidor da Únicopag, não do navegador do aluno, e usar o deles envenenaria a correspondência em vez de melhorá-la.
...
**A coordenada vem sempre do Nominatim, nunca da BrasilAPI.** A BrasilAPI v2 tem um campo `location.coordinates` que parecia dar a coordenada exata do CEP — até um teste ao vivo mostrar que ela devolveu a **mesma** coordenada para dois bairros do Rio a vários quilômetros de distância (São Cristóvão e Jacaré): um dos provedores internos que ela consulta ("open-cep") retorna um ponto genérico em vez de recusar quando não tem o dado real, e nada na resposta avisa que aquele valor é chute. Confiar nele fazia inscrições de bairros diferentes caírem exatamente no mesmo pixel do mapa — e não porque faltava geocodificar, e sim porque a fonte "exata" mentia. `lib/cep.ts` para de ler esse campo de propósito; `PUT /api/triagem` sempre geocodifica bairro + cidade + UF pelo [Nominatim](https://nominatim.openstreetmap.org/) (`coordenadaAproximada`) antes de gravar a resposta do passo 1. Não é a rua do aluno, é o centro da área que ele informou — mas é uma fonte verificada, e mais honesto que uma coordenada "exata" errada. Essa busca roda na gravação da triagem, não na tela de CEP em si: o aluno não espera por ela, porque `save()` no `triage-flow.tsx` já é fire-and-forget.
...
- **Resumo e funil completo** — quantos visitaram a landing, quantos entraram, quantos pagaram, quantos matricularam de fato (triagem concluída), e a contagem em cada ponto de queda: só preencheu os dados, abriu cobrança e não pagou, pagou e não terminou a triagem, cancelou, ou teve cobrança recusada/expirada/estornada. É a resposta direta para "quem tem entrado e quem realmente tem feito matrícula". A visita na landing é o único número desse bloco que não vem de `inscricoes` — ela acontece antes de existir qualquer inscrição para ancorar, e por isso mora numa tabela à parte (`visitas_landing`, migration `0011`), gravada por `POST /api/visitas` a partir de `trackLandingView` em `lib/checkout.ts`. É o mesmo `PageView` que o pixel do Meta já contava do lado dele — sem essa tabela, o número simplesmente não existia em lugar nenhum nosso, e "visita → preencheu os dados" era invisível dentro do próprio painel.
- **Mapa de origem** — um ponto aproximado por inscrição, na região do CEP/endereço informado na triagem (ver [CEP na triagem](#cep-na-triagem) acima para a fonte da coordenada). A cor é por tempo, não por status: verde forte para quem acabou de entrar, esmaecendo para laranja fraco ao longo de 8 semanas — dá para ver o mapa "preenchendo" e onde uma região ficou parada.
