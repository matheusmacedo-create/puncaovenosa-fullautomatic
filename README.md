...
O funil usa o projeto Supabase **RedacaoCruzVermelha-Rj** (ref `wlbbfkudeibalkaqphpo`, região `ca-central-1`), o mesmo da redação, no schema `public`. Foi uma consolidação deliberada: dois projetos Supabase para uma instituição só era um a mais para pagar, monitorar e esquecer. As nove tabelas, as oito funções, os seis tipos e a sequence do funil convivem com as tabelas da redação sem colisão de nome (conferido antes de migrar). A migration `0016` faz a blindagem que um projeto compartilhado exige: tira `anon`/`authenticated` de tudo que é do funil (a chave publicável da redação está no bundle do navegador dela) e concede ao `service_role`, explicitamente, o que o servidor precisa.

O projeto antigo (`puncaovenosa-fullautomatic`, ref `lqpnbqislaxzhqkszijg`, `sa-east-1`) fica como arquivo até a troca ser confirmada em produção. A cópia de dados foi idempotente (`on conflict do nothing` por `id`); a única coisa que não veio foi o corpo (`payload`) das entregas da Conversions API em `meta_capi_entregas` — PII com hash, IP e user-agent do aluno, que não há por que replicar em outro banco; o registro de cada tentativa (evento, resultado, erro) veio inteiro.

**Custo assumido:** o projeto da redação está no Canadá, e a função da Vercel em São Paulo. Cada ida ao banco paga ~130 ms a mais do que pagava em `sa-east-1`. No `POST /api/pagamentos` são duas idas (≈ 0,3 s a mais num total dominado pelos 9 a 22 s da Únicopag); em `/secretaria`, que faz várias consultas em série, o efeito é mais visível. Se um dia isso pesar, a saída é recriar o projeto da redação em `sa-east-1`, não voltar a ter dois bancos.

**Troca das variáveis (a única etapa manual):** na Vercel, o projeto precisa passar a apontar para o banco novo. Se as variáveis vêm da integração Supabase (Storage → Supabase), conecte o recurso `RedacaoCruzVermelha-Rj` ao projeto e desconecte o antigo; se foram definidas à mão, troque `NEXT_PUBLIC_SUPABASE_URL` (ou `SUPABASE_URL`) para `https://wlbbfkudeibalkaqphpo.supabase.co` e `SUPABASE_SECRET_KEY` (ou `SUPABASE_SERVICE_ROLE_KEY`) pela chave secreta do projeto novo. Depois, **Redeploy**. Inscrições criadas no banco antigo entre a cópia e a troca são trazidas por uma nova rodada da mesma cópia (é idempotente) — peça isso logo depois de trocar.
